...
function onCreate()
    makeLuaSprite('AceOfHearts','crossed-out/AceOfHearts', 100, 490)
    ...
    makeLuaSprite('FullAce','crossed-out/FullAce', -50, 0)
    ...
    makeAnimatedLuaSprite('SansEvent_Dodge','crossed-out/SansEvent_Dodge', 640, 300)
	...
	makeAnimatedLuaSprite('SansEvent_Dodge_Hit','crossed-out/SansEvent_Dodge', 640, 300)
...
function goodNoteHit(id, noteData, noteType, isSustainNote)
    ...
    elseif (curNotesLoaded < cupheadCardAlphaReady) then
        ...
        if (CardIsAllowedOnScreen == true) then
            playSound('crossed-out/cuphead_CardIsReady', 5)
        end
    end
end
...
function onUpdate(elapsed)
    ...
    if (CardIsAllowedOnScreen == true) and (canAttackCard == true) and (keyJustPressed('space')) then
        ...
        if not lowQuality then
            playSound('crossed-out/cuphead_CardAttack', 5)
        end
    end
    ...
    if sansCanDoDodge == true and keyJustPressed('space') then
      addScore(1000);
		...
		if not lowQuality then
			playSound('crossed-out/SansBoneDodge_Dodge', 0.7)
		end
...
function onEvent(name, value1)
    ...
    if name == 'Sans_BoneDodge' then 
        ...
        canAttackCard = false
		playSound('crossed-out/SansBoneDodge_Appear', 1)
...
function onBeatHit()
	if sansBoneDodgeTime == 0 and sansMustDoDodge == true then 
        addScore(-1000);
		if not lowQuality then
			playSound('crossed-out/SansBoneDodge_Miss', 1)
		end
        canAttackCard = sansDodgeSaveCard
		removeLuaSprite('SansEvent_Dodge', false)
	...
	elseif sansBoneDodgeTime > 0 then
		if not lowQuality then
			playSound('crossed-out/SansBoneDodge_Damage', 0.5)
		end
