...
function bullInit(i)
	...
	if (t == 'blaster') then
		...
		stopSound('blaster');
		playSound('crossed-out/blaster', 0.3, 'blaster');
	...
	elseif (t == 'jkblaster') then
		...
		setProperty(n..'.alpha', 1);
		stopSound('blaster');
		playSound('crossed-out/blaster', 0.5, 'blaster');
	...
	elseif (t == 'blasterInvis') then
		...
		setProperty(n..'.alpha', 1);
		stopSound('rise');
		playSound('crossed-out/rise', 1, 'rise');
	...
	elseif (t == 'GasterWarnBase') then
		...
		if (bProp[n..'.exArg'] == nil) then bProp[n..'.exArg'] = 1 end
			...
			playSound('crossed-out/GasterHandAppear', 1, 'GasterHandAppear');
...
function bullSpawn(type, xx, yy, hsp, vsp, dmg, ang, exArg, exArg2)
	...
	makeLuaSprite(name, 'crossed-out/ut/bullets/'..spr, boxX + xx, boxY + yy);
...
function bullUpdate(elapsed)
	...
	if (hRemove) then
		if (sndTime <= 0) then 
			playSound('crossed-out/snd_hurt1', 1);
			sndTime = 1;
		else
	...
	if (tp == 'blaster') then
		...
		elseif (s == 1) then
			...
			if (t == 48) then
				...
				makeLuaSprite(n, 'crossed-out/ut/bullets/blasterOpen', boxX + bProp[n..'.x'], boxY + bProp[n..'.y']);
				...
				playSound('crossed-out/blast_shoot', 0.5, 'blast_shoot');
		...
		elseif (tp == 'blasterInvis') then
			...
			elseif (s == 1) then
				...
				if (t == 48) then
					...
					removeLuaSprite(n, true);
					makeLuaSprite(n, 'crossed-out/ut/bullets/blasterInvisOpen', boxX + bProp[n..'.x'], boxY + bProp[n..'.y']);
					setProperty(n..'.offset.x', getProperty(n..'.width') / 2);
					setProperty(n..'.offset.y', getProperty(n..'.height') / 2);
					setProperty(n..'.antialiasing', false);
					addLuaSprite(n, true);
					...
					playSound('crossed-out/sansattack', 1, 'sansattack');
		...
		elseif (tp == 'GasterBlastBase') then
				...
				elseif (s == 1) then
					...
					if (t == 90) then
						...
						playSound('crossed-out/GasterHandShoot', 0.5, 'GasterHandShoot');
...
function onCreate()
	...
	makeLuaSprite('p', 'crossed-out/ut/soul', boxX, boxY);
...
function onUpdate(elapsed)
	...
	if dogModesteps == 3 then
		dogModeactive = true
		playSound('crossed-out/Undertale_DogMode', 4)
		dogModesteps = 4
	...
	if (ground and heavy) then
		triggerEvent('Screen Shake', '0.2,0.005', '0.2,0.005');
		playSound('crossed-out/impact', 1);
		act = 1;
		heavy = false;
	end
	...
	for i = 0, getProperty('eventNotes.length') - 1, 1 do
		...
		if (curStep == math.floor(sTime - 8) or curStep == math.floor(sTime - 4) or curStep == math.floor(sTime - 2)) then
			if ((name == '3_boxOpen' and canMove == false) or name == '3_boxClose') then
				--stopSound('notice');
				--playSound('crossed-out/notice', 1, 'notice');
				notice = 10;
			end
		end
	end
...
function onSongStart()
	...
	if dogModeactive == false then
		if keyPressed('space') then
			...
			playSound('crossed-out/Undertale_EasyMode', 4)
			if (difficultyName == 'Easy') then
				dmgMulti = 0.25
			elseif (difficultyName == 'Normal') or (difficultyName == 'Omegaspace') then
				dmgMulti = 0.33
