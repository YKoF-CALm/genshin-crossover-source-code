...
function onCreate()
	...
	makeAnimatedLuaSprite('Cuphead Hadoken','crossed-out/Cuphead Hadoken', -100, 500)
	addAnimationByPrefix('Cuphead Hadoken','Hadolen instance ','Hadolen instance ',24, false)
	scaleObject('Cuphead Hadoken', 0.65, 0.65);	

	makeAnimatedLuaSprite('Cuphead Hadoken Final','crossed-out/Cuphead Hadoken Final', -100, 500)
	...
	makeAnimatedLuaSprite('Cuphead Burst','crossed-out/Cuphead Hadoken', 300, 0)
...
function opponentNoteHit(id, noteData, noteType, isSustainNote)
	if noteType == 'CupNote_BigShoot' then
		if not lowQuality then
			--playSound('crossed-out/CupheadPreBigShot', 3);
		end
        ...
        if (dadName == 'ganyu-indie-mad-dark') or (dadName == 'ganyu-crossed-out') then
            characterPlayAnim('dad', 'attack', true)
            runTimer('sound', 0.4, 1)
		elseif (gfName == 'ganyu-indie-mad') then
            characterPlayAnim('gf', 'attack', true)
            runTimer('sound', 0.4, 1)
		end
...
function onTimerCompleted(tag)
	...
	if tag == 'CupheadBigShot_ShotConnect' then
		...
		playSound('crossed-out/CupheadBigShot', 3);
		if not lowQuality then
