...
function onCreate()
	makeAnimatedLuaSprite('fx', 'phantasm/vintage', 0, 0)
	...
	for i = 1,2 do
		makeLuaSprite(bgs[i], 'phantasm/'..bgs[i], 0, 0)
...
function fleetON()
	...
	playSound('stat', 0.3)
	triggerEvent('Change Character', 0, 'kazuha-player')
end
...
function fleetOFF()
	...
	doTweenAlpha('fleetON', 'fx', 0, 1, 'linear')
	playSound('stat', 0.3)
	triggerEvent('Change Character', 0, 'aether-duality-player')
end
